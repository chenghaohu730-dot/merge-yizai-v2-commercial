...
if (!specPath) {
  console.error("Usage: node tools/yunwu-edit-image.mjs <spec.json>");
  process.exit(1);
}
...
form.append("size", spec.size || "1024x1024");
form.append("quality", spec.quality || "high");
for (const key of ["background", "output_format", "output_compression", "moderation"]) {
  if (spec[key] !== undefined) form.append(key, String(spec[key]));
}

const imagePaths = Array.isArray(spec.image) ? spec.image : [spec.image];
for (const imagePath of imagePaths.filter(Boolean)) {
  const blob = await fs.openAsBlob(path.resolve(imagePath), { type: "image/png" });
