...
const processedRoot = path.join(projectRoot, "art", "final", "v2-local-assets");
const checksRoot = path.join(projectRoot, "tests", "device-checks", "visual-v2-local-assets");
...
await writeFile(path.join(processedRoot, "asset-processing-report.json"), JSON.stringify({
  ...
  avatarReport,
  notes: [
    "原始 PNG 均为不带透明通道的棋盘格底图，已按边界连通背景抠底。",
...
async function backupRuntimeAssets() {
  const backupRoot = path.join(projectRoot, "art", "backup", `pre-v2-local-assets-${stamp()}`);
...
async function processUiAssets(files) {
  ...
  const direct = [
    ...
    ["button_restart", pick("14_19_43 (1)")],
    ["control_sound", pick("14_19_44 (2)")],
    ["control_pause", pick("14_17_04 (10)")],
    ["control_restart", pick("14_19_43 (1)")],
  ...
  return {
    ...output,
    gameTop: pick("14_08_23 (1)"),
    gameLeft: pick("14_08_23 (2)"),
    gameRight: pick("14_08_23 (3)"),
    gameGlass: pick("14_08_24 (4)"),
    gameBase: pick("14_08_24 (5)"),
    gameControls: pick("14_08_25 (6)"),
...
async function buildGameBackground(ui) {
  const W = 750;
  const H = 1334;
  const top = await trimToSubject(await removeCheckerBackground(ui.gameTop, { threshold: 222, removeAllLightBg: false }), 8);
  const left = await trimToSubject(await removeCheckerBackground(ui.gameLeft, { threshold: 222, removeAllLightBg: false }), 4);
  const right = await trimToSubject(await removeCheckerBackground(ui.gameRight, { threshold: 222, removeAllLightBg: false }), 4);
  const glass = await clearLightBackground(
    await trimToSubject(await removeCheckerBackground(ui.gameGlass, { threshold: 222, removeAllLightBg: false }), 8),
    214,
  );
  const base = await trimToSubject(await removeCheckerBackground(ui.gameBase, { threshold: 222, removeAllLightBg: false }), 8);
  const controls = await trimToSubject(await removeCheckerBackground(ui.gameControls, { threshold: 222, removeAllLightBg: false }), 8);
  ...
  const bg = await sharp({
    ...
    .composite([
      ...
      { input: await fitPng(top, 706, 236), left: 22, top: 0 },
      { input: await fitPng(base, 730, 186), left: 10, top: H - 206 },
      { input: await fitPng(controls, 390, 88), left: 180, top: H - 120 },
    ])
  ...
  const frame = await sharp({
    ...
    .composite([
      { input: await fitPng(left, 86, 948), left: 18, top: 204 },
      { input: await fitPng(right, 86, 948), left: W - 104, top: 204 },
      { input: await fitPng(glass, 646, 846), left: 52, top: 204 },
      { input: await fitPng(warning, 540, 24), left: 105, top: 300 },
    ])
...
async function cleanupRuntimeUiExtras() {
  for (const name of ["button_restart.png", "merge_bubble.png", "next_frame.png", "progress_bar.png", "warning_line.png"]) {
