...
## 2026-07-22：阶段 1 本地分包骨架与旧视觉占位构建
...
- 旧首页、游戏背景和结算面板曾由 SpriteFrame 载入；点击区域和场景切换由程序负责。该记录只证明图片加载与构建链，后续用户确认这些旧图全部作废，不能作为 V2 商业美术或 Golden Slice。
...
- 阶段 1 仍未达到退出条件：Game 场景目前是可切换的旧视觉占位外壳，不是商业 Golden Slice；玩法物理组件和真实默认局尚未完整挂载，微信开发者工具/真机也未验收。
...
- 曾将旧首页/局内候选画面接入 Cocos 场景并留存 430×765 浏览器截图。用户随后明确否决该视觉来源；这些截图只保留为错误基线证据，不能用于方向确认或完成声明。
...
- 旧 Golden Slice 资格已撤销；必须先用 Grok 从零制作 4–6 套新风格方向，再由用户确认。后续 UI Kit、原创默认头像链和批量皮肤生产不得提前启动。
- 全量界面、头像 fallback、粒子和完整任务/商店/排行视觉替换尚未完成。
- 音频真机听感、切后台/来电/静音恢复仍待设备验收。

## 2026-07-22：V2 美术全量重制口径纠偏

### 用户锁定决策

- 已发布 V1 只作玩法行为、数值、平台适配、测试方法和代码实现参考。
- V1、旧 `art/final`、当前旧 Cocos 中的背景、UI、按钮、面板、头像、皮肤、图标、粒子、宣传图、旧音频和既有风格全部禁止进入 V2。
- 禁止通过复制、缩放、改色、裁切、拼接、高清重绘、图生图或风格迁移变相继承旧图。
- V2 美术风格重新探索，可以对标《开心消消乐》等头部休闲游戏的商业完成度，但不得复制其可识别角色、Logo、图标、布局、按钮、文案或商业外观。
- 所有涉及亿仔的生成和编辑，只使用本机 `art/source/yizai_highest_source.png` 作为唯一正式身份参考。

### 本轮文档修正

- 新增 `docs/13_《合成亿仔》V2.0美术全量重制硬性边界.md`。
- 新增 `art/prompts/v2-commercial/00_art_bible/《合成亿仔》V2.0美术圣经_v2.md`。
- 将美术圣经 v1、旧色板和旧首页/局内提示词标记为废止，禁止继续执行。
- 同步修正 AGENTS、总控、美术资源需求表、Golden Slice 说明、V1 引用边界、Cocos 资源策略、V2 状态和 TASKS。
- 撤销“旧候选画面已接入即等于 Golden Slice 完成”的错误状态；当前正式 V2 Grok 美术仍为未完成。

### 当前真实门禁

- 下一步必须先重新探测 Grok CLI，并确认亿仔任务能真实读取本机参考图。
- 先提交 4–6 套新风格 Golden Slice；用户未批准前禁止批量生产和导入 Cocos。
- 当前旧图可留在仓库用于 V1 追溯，但后续需要由导入验证器拒绝进入正式 V2 Bundle。
